  ...
  return (
    <header
      className={`sticky top-0 z-40 w-full transition-all duration-300 ${
        scrolled
          ? "bg-midnight-navy/95 backdrop-blur-md shadow-lg shadow-black/20"
          : "bg-midnight-navy"
      }`}
    >
      <div className="max-w-content mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
            ...
            {navLinks.map((link) =>
              link.dropdown ? (
                <div key={link.label} className="relative group">
                  ...
                  <div className="absolute top-full left-0 mt-1 w-64 bg-deep-slate border border-white/10 rounded-xl shadow-xl shadow-black/40 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 py-2">
                    {link.dropdown.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="block px-4 py-2 text-sm text-pale-gray hover:text-white hover:bg-white/5 transition-colors"
                      >
                        {item.label}
                      </Link>
                    ))}
          ...
          <div className="hidden lg:flex items-center gap-3">
            <Link
              href="/free-audit"
              className="bg-electric-blue hover:bg-blue-600 text-white text-sm font-semibold px-4 py-2 rounded-full transition-colors"
            >
              Free Audit
            </Link>
          </div>
